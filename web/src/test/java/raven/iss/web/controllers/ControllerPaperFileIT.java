...
public class ControllerPaperFileIT extends AbstractRestControllerTest {
    ...
    public void uploadAndGetTest() throws Exception {
        ...
        getMockMvc().perform(multipart("/api/conferences/2/papers/2/file").file(multipartFile)
                .contentType(MediaType.APPLICATION_JSON)
                .header("Authorization", "Bearer " + token))
                .andExpect(status().isOk());

        MockHttpServletResponse response = getMockMvc().perform(get("/api/conferences/2/papers/2/file")
                .contentType(MediaType.APPLICATION_JSON)
                .header("Authorization", "Bearer " + token))
                .andExpect(status().isOk())
